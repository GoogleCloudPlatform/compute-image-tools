...
func validateLicense(osDisk *compute.AttachedDisk, sourceOS, targetOS string) error {
	...
	if !matchSourceOSVersion {
		return daisy.Errf("No valid Windows Server PayG license can be found. Any of the following licenses are required: %v", upgradePaths[sourceOS][targetOS].expectedCurrentLicense)
	}
	if upgraded {
		return daisy.Errf("The GCE instance has the %v license attached. This likely means the instance either has been upgraded or has started an upgrade in the past.", upgradePaths[sourceOS][targetOS].licenseToAdd)
	}
	return nil
}
