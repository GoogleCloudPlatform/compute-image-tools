...
COPY / /build
WORKDIR /build
RUN cd /build/cli_tools_tests/e2e_tests/gce_ovf_import && CGO_ENABLED=0 go build -o /gce_ovf_import_test_runner
...
COPY /cli_tools_tests/e2e_tests/gce_ovf_import/scripts/ /scripts/
