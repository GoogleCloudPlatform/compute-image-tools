...
func validateSourceFile(storageClient commondomain.StorageClientInterface) error {
	...
	if err == nil {
		return fmt.Errorf("cannot import an image from a compressed file. Please provide a path to an uncompressed image file. If the compressed file is an image exported from Google Compute Engine, please use 'image create' instead")
	}
