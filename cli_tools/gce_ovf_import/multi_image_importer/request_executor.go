...
func (r *requestExecutor) executeRequests(parentContext context.Context, requests []importer.ImageImportRequest) (imageURIs []string, err error) {
	...
	for _, request := range requests {
		if _, err := r.computeClient.GetImage(request.Project, request.ImageName); err == nil {
			return imageURIs, daisy.Errf("Aborting import: Intermediate image %s already exists", request.ImageName)
		}
	}
	for _, request := range requests {
		req := request
		imageURIs = append(imageURIs, fmt.Sprintf("projects/%s/global/images/%s", request.Project, request.ImageName))
		logPrefix := fmt.Sprintf("[import-%s]", req.DaisyLogLinePrefix)
		group.Go(func() error {
			return r.singleImporter.ImportImage(ctx, req, r.logger.NewLogger(logPrefix))
		})
	}
	err = group.Wait()
	if err != nil {
		r.deleteImagesIfCreated(requests)
	}
	return imageURIs, err
}

func (r *requestExecutor) deleteImagesIfCreated(requests []importer.ImageImportRequest) {
	for _, request := range requests {
		if _, err := r.computeClient.GetImage(request.Project, request.ImageName); err == nil {
			r.logger.Debug("Found image " + request.ImageName)
			if err = r.computeClient.DeleteImage(request.Project, request.ImageName); err != nil {
				r.logger.User(fmt.Sprintf("Failed to delete %q. Manual deletion required.", request.ImageName))
			} else {
