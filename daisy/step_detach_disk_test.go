...
func TestDetachDisksValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{"instance name + device name case (instance created in workflow, device attached in workflow)", &DetachDisks{{Instance: testInstance, DeviceName: testDisk}}, false},
	}
	for _, tt := range tests {
		w := testWorkflow()
		...
		sampleDisk := compute.Disk{Name: testDisk}
		w.instances.regURL(instanceRegisterName, false)
		w.disks.regAttach(sampleDisk.Name, instanceRegisterName, "", as)
