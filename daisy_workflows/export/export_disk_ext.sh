...
# Verify VM has network access to Compute and Storage API endpoints
for endpoint in "https://compute.googleapis.com" "https://storage.googleapis.com"
do
  STATUSCODE=$(curl --head --silent --write-out '%{http_code}'  --output /dev/null $endpoint)
  if [ $STATUSCODE -ge 200 ] ; then
    continue
  else
    echo "ExportFailed: Cannot access Google APIs at $endpoint. Ensure that VPC settings, including DNS, Firewall Rules, and Private Google Access, allow VMs to access Google APIs."
    exit
  fi
done
