...
try {
  ...
  Write-Output "BCD drive: ${bcd_drive}"
  Write-Output "OS drive: ${script:os_drive}"
  ...
  New-Item $driver_dir -Type Directory
  New-Item $script:components_dir -Type Directory
  ...
  # Add-WindowsDriver results in errors on 32-bit (all or just Windows 7, I can't remember)
  ...
  # Skip this step until Google provides 32-bit googet packges through a repository.
