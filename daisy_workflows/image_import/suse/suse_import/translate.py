...
Parameters (retrieved from instance metadata):
  ...
  distro: One of 'opensuse' or 'sles'.
  major_version: Major version such as '12' or '15'.
  minor_version: Minor version such as '1' or '5'.

For distro, major_version, or minor_version, if the value is unspecified,
it will be populated with a detected value.
"""
...
def _get_release(g: guestfs.GuestFS) -> _SuseRelease:
  ...
  distro = utils.GetMetadataAttribute('distro').lower()
  major = utils.GetMetadataAttribute('major_version').lower()
  minor = utils.GetMetadataAttribute('minor_version').lower()
  ...
  matched = None
  for d in _releases:
    if re.match(d.flavor, distro) \
        and re.match(d.major, major) \
        and (not d.minor or re.match(d.minor, minor)):
      matched = d
  if not matched:
    supported = ', '.join(
        ['{}-{}.{}'.format(d.flavor, d.major, d.minor) for d in _releases])
    raise ValueError(
...
def translate():
  ...
  subscription_model = utils.GetMetadataAttribute('subscription_model').lower()
