...
[CmdletBinding()]
  param (
    ...
    [String]$ProductKey = 'WMDGN-G9PQG-XVVXX-R3X43-63DFG',
...
$script:UPGRADABLE_WIN_VERSION_MAX = New-Object -TypeName System.Version -ArgumentList 10,0,14393,65535  # Win 2016, final build

$script:ACCEPTABLE_WIN_VERSIONS_MIN = New-Object -TypeName System.Version -ArgumentList 10,0,17763,0     # Win 2019
$script:ACCEPTABLE_WIN_VERSIONS_MAX = New-Object -TypeName System.Version -ArgumentList 10,0,20347,65535 # The last 2019 build is not known yet
...
try {
    ...
    if ($script:installationType.InstallationType -eq 'Server Core') {
        $script:imageIndex = Find-ImageIndex -image_name $('Windows Server 2019 SERVERDATACENTERCORE', 'Windows Server 2019 Datacenter')
    }
    elseif ($script:installationType.InstallationType -eq 'Server') {
        $script:imageIndex = Find-ImageIndex -image_name $('Windows Server 2019 SERVERDATACENTER', 'Windows Server 2019 Datacenter (Desktop Experience)')
    }
    else {
    ...
    elseif (([System.Environment]::OSVersion.Version -ge $script:ACCEPTABLE_WIN_VERSIONS_MIN) -and
        ...
        throw "This version of Windows ($script:winVersion) can not be upgraded to Windows Server 2019"
    }
}
catch {
    Write-LogError $_
